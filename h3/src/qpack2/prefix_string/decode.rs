    ...
    fn parse_progress<B: Buf>(
        ...
        loop {
            ...
            let current_byte = match self.current_byte {
                Some(8) => {
                    // If the current byte is 8, it means we have read all bits
            ...
            // the bit position can only be between 0 and 7
    ...
    fn get_next_byte(&mut self, buf: &mut impl Buf) -> GetNextByteResult {
        if self.bytes_read >= self.length {
            ...
            if let Some(byte) = self.current_byte {
                if (byte & (0b1111_1111 >> self.current_bit_pos))
...
mod tests {
    ...
    use super::*;
    use crate::tests::{all_chunking_combinations, test_all_chunking_combinations};
