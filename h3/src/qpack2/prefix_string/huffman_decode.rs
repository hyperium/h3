...
enum GetNextByteResult {
    ...
    Error(HuffmanDecodingError),
}

impl StatefulParser<HuffmanDecodingError, Vec<u8>> for StatefulHuffmanDecoder {
    fn parse_progress<B: Buf>(
        mut self,
        reader: &mut B,
    ) -> ParseProgressResult<Self, HuffmanDecodingError, Vec<u8>> {
        loop {
            // Gets the current byte or tries to read the next byte
            // There is no Option::get_or_try_insert_with
            ...
            let current_byte = if self.current_bit_pos == 8 {
                // If the current bit position is 8 or more, we need to read the next byte
                match self.get_next_byte(reader) {
                    GetNextByteResult::Byte(value) => {
            ...
            } else if self.current_bit_pos > 8 {
                // We read new byte last iteration. We just need to update bit Position
            ...
            let value = if self.current_decoder.lookup > (8 - self.current_bit_pos) {
                ...
                // Shift to left to remove the already read bits
                let value = value << self.current_bit_pos;

                // allign bits to the right
                let value = value >> (16 - self.current_decoder.lookup);
                value as u8
            } else {
                // Enough bits, so we can read the value directly

                // Remove the already read bits from the current byte
                let current_byte = current_byte << self.current_bit_pos;
                // allign bits to the right
                current_byte >> (8 - self.current_decoder.lookup)
            };
            ...
            match self.current_decoder.table.get(value as usize) {
                ...
                None => return ParseProgressResult::Error(HuffmanDecodingError::EOS),
            }
        }
    }
}
    ...
    fn get_next_byte(&mut self, buf: &mut impl Buf) -> GetNextByteResult {
        if self.bytes_read >= self.length {
            ...
            if let Some(byte) = self.current_byte {
                // TODO: Check padding: encoder must be specific only in 11111111111111111111111
                if self.current_bit_pos < 8
                    && (byte & (0b1111_1111 >> self.current_bit_pos))
                        != (0b1111_1111 >> self.current_bit_pos)
                {
                    return GetNextByteResult::Error(HuffmanDecodingError::PaddingError);
                }
            }
...
mod tests {
    ...
    use super::super::huffman_encode::HpackStringEncode;
    use super::*;
    use crate::tests::{
    ...
    macro_rules! decoding {
        [ $( $code:expr => $( $byte:expr ),* ; )* ] => { $( {
            ...
            test_all_chunking_combinations(
                ...
                || StatefulHuffmanDecoder::new(bytes.len()),
                false,
                ParseProgressResult::Done(vec![$code]),
            );
        } )* }
    }
    ...
    fn test_decode_all_code_joined() {
        ...
        test_all_chunking_combinations(
            ...
            || StatefulHuffmanDecoder::new(len),
            true,
    ...
    fn decode_common_header_values() {
        ...
        for s in cases {
            let encoded = s.as_bytes().to_vec().hpack_encode();
            test_all_chunking_combinations(
