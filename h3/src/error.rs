...
pub enum H3ConnectionError {
    /// Error from the QUIC layer
    PeerError(ConnectionErrorIncoming),
    /// The local implementation has thrown this error
    LocalError(ConnectionClosedKind),
}

#[derive(Clone, Debug)]
/// Error type which represents the reason for a connection being closed by this library
/// This types is generated by this library when it encounters an error
pub enum ConnectionClosedKind {
    /// encountered a error with the h3 protocol
    #[non_exhaustive]
    Application {
        /// The error code
        code: Code,
    ...
    /// Encountered a header that is too big
    /// 
    /// This is not normal h3 error, but a normal http error response as specified in the rfc
    #[non_exhaustive]
    HeaderTooBig {
